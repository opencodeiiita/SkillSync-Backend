...
router.post("/profile/:id/portfolio", authenticateUser, async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    res.status(201).json({ message: "Portfolio item added successfully", portfolio: userProfile.portfolio });
  } catch (error) {
...
router.put("/profile/:id/portfolio/:portfolioId", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const portfolioIndex = userProfile.portfolio.findIndex(item => item._id.toString() === portfolioId);
    if (portfolioIndex === -1) {
    ...
    res.status(200).json({ message: "Portfolio item updated", portfolio: userProfile.portfolio });
  } catch (error) {
    res.status(500).json({ message: "Server error", error });
  }
});
